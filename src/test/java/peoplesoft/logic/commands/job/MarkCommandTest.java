...
public class MarkCommandTest {
    ...
    public void constructor_nullArgs_throwsNullPointerException() {
        assertThrows(NullPointerException.class, () -> new MarkCommand(null, new Employment()));
        assertThrows(NullPointerException.class, () -> new MarkCommand(Index.fromOneBased(1), null));
    }

    @Test
    public void execute_nullModel_throwsNullPointerException() {
        assertThrows(NullPointerException.class, () -> new MarkCommand(Index.fromOneBased(1), new Employment())
                .execute(null));
    }
    ...
    public void execute_incorrectArgs_throwsCommandException() {
        // No job at index 3
        MarkCommand cmd = new MarkCommand(Index.fromOneBased(3), new Employment());
    ...
    public void execute_correctArgs_success() {
        ...
        MarkCommand cmd = new MarkCommand(Index.fromOneBased(1), emp);
        assertCommandSuccess(cmd, model, String.format(MarkCommand.MESSAGE_SUCCESS, PAID.getDesc(), "paid"),
                expectedModel);

        // Mark as not paid
        expectedModel.setJob(PAID, UNPAID);
        expectedModel.setPerson(aliceToo, ALICE);
        cmd = new MarkCommand(Index.fromOneBased(1), emp);
        assertCommandSuccess(cmd, model, String.format(MarkCommand.MESSAGE_SUCCESS, UNPAID.getDesc(), "not paid"),
                expectedModel);
    }
}
